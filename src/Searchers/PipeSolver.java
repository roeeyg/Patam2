...
public class PipeSolver implements Searchable<char[][]>  {
	...
	public PipeSolver(MyState state ,int rows,int cols) {		
		this.rows = rows;
		this.cols = cols;
	...
	public State<char[][]> getInitState() {
	...
	public ArrayList<State<char[][]>> getAllStates(State<char[][]> state) {
		ArrayList<State<char[][]>> result = new ArrayList<>();
		for	(int i=0;i<this.rows;i++)
		{
			for	(int j=0;j<this.cols;j++)
			{
                ...
                result.add(new MyState(newState, this.rows));
		}	
		}
		return result;
	}
